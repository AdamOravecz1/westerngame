...
@onready var enemy_anim: AnimationPlayer = $Boss.get_node("AnimationPlayer")
@onready var animation_tree: AnimationTree = $Boss/AnimationTree

@onready var gun_ray := $Boss/Armature/Skeleton3D/Gun/RayCast3D
...
func _ready() -> void:
	...
	skeleton = $Boss/Armature/Skeleton3D/PhysicalBoneSimulator3D
	model = $Boss
...
func shoot_gun():
	for i in $Boss/Armature/Skeleton3D/Gun/MuzzleFlash.get_children():
		i.emitting = true
...
func delete_after_death():
	...
	for bone in $Boss/Armature/Skeleton3D.get_children():
